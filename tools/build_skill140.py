...
SPECS=[
 ...
 dict(id=19123,name='트라이스파이크 프로스트',key='mt_frost',peer=10111,branch=2,hits=3,area=6,mp=280,cool=10,damage=[1120,1230])]
...
def effects(config=None):
 ...
 final_resources=[dict(target=Path(rel).name,frames=len(read_wed(data)['frames'])) for rel,data in compiled.items() if rel.startswith('Effect/')]
