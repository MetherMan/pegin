...
for(const [key,url] of Object.entries(data.textures)){const t=await loader.loadAsync(url);t.flipY=false;t.colorSpace=T.SRGBColorSpace;textures[key]=t;}
let entries=[],selected='hellhorse',motion='idle',paused=false,time=0;
...
function load(key){
 ...
 for(const chunk of data[key]){const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(chunk.corners.flatMap(c=>chunk.points[c[0]]),3));g.setAttribute('uv',new T.Float32BufferAttribute(chunk.corners.flatMap(c=>c.slice(4,6)),2));const mesh=new T.Mesh(g,new T.MeshBasicMaterial({map:textures[key==='original'?'original':'hellhorse'],side:T.DoubleSide}));mesh.frustumCulled=false;group.add(mesh);entries.push({chunk,mesh});}
 document.querySelector('#hell').classList.toggle('active',key==='hellhorse');document.querySelector('#original').classList.toggle('active',key==='original');update();
}
function view(name){
...
document.querySelector('#hell').onclick=()=>load('hellhorse');document.querySelector('#original').onclick=()=>load('original');
...
load(selected);view('quarter');document.querySelector('#status').textContent='718 삼각형 · 원본 32본 · 원본 대기 / 달리기';
