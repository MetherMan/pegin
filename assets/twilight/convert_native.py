...
assert sign in [-1,1]
groups={};colors=[];texture_names={};baked_images={}
...
pos,normal,uv,atlas_report=single_mesh_atlas(groups,baked_images,pack_texture,P/'mt_twilight_atlas.wtm')
# MOD keeps normals/UVs per corner. Weld only positions to stay below the
# legacy vertex limit without losing UV seams or changing any blade geometry.
ps,indices=np.unique(pos,axis=0,return_inverse=True);nv=len(ps);nf=len(pos)//3
assert nv<65536 and nf<65536
texture='mt_twilight_atlas.bmp'
corners=np.empty(len(pos),dtype=np.dtype([('i','<i4'),('n','<f4',3),('uv','<f4',2)]));corners['i']=indices;corners['n']=normal;corners['uv']=uv
roughness=appearance['metalRoughness'] if appearance else .38
...
chunks=[chunk];report=[dict(texture=texture,vertices=nv,triangles=nf)]
