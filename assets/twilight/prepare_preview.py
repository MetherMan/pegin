...
if 'const blobs=' not in s:
 ...
 tail=tail[:a]+"const info={name:'푸른달의 트와일라잇 양손검',rank:9,name_color:'#72CFFF',rank_color:'#348BFF',type:2,triangles:stats.reduce((a,b)=>a+b.triangles,0),mesh_objects:stats.length,cloth_strips:4,source:'concept_texture.png',not_in_game:true,construction:'Real extruded blade and separate crescent fins, open negative spaces, round wrapped hilt, curved cloth ribbon meshes. Both blade faces use the concept projection; some painted lighting remains in the texture.',parts:stats};\n"+tail[b:]
 tail=tail.replace('chain_links:count','cloth_strips:4')
 s+=tail
p.write_text(s,encoding='utf-8')
spec={'name':'푸른달의 트와일라잇 양손검','rank':9,'weapon_type':2,'weapon_class':'양손검','name_color':'#72CFFF','rank_color':'#348BFF','inventory_style':'black background, antique gold border','damage':None,'in_game_registered':False,'stage':'3D model and inventory preview','source_concept':'concept_texture.png','icon':'inventory_icon_imagegen.png'}
...
def viewer():
 ...
 html=html.replace('#4a2831','#163359').replace('#a0606f','#348bff').replace('color:#ede8e4','color:#72cfff').replace('color:#a6a3a5','color:#8dacc6')
