...
def custom_test():return b'\x66\x81\x3f'+struct.pack('<H',19030)+b'\x72\x07\x66\x81\x3f'+struct.pack('<H',19050)+b'\x76'
...
code+=b'\xc7\x80'+struct.pack('<i',0x45e60-pc)+struct.pack('<I',0xff348bff)
...
for base in [0x10000000,0x03500000]:
 ...
 for rank in range(1,21):
  for item in [11445,19000,19029,19030,19040,19050,19051]:
   default=[0xffc2c2c2,0xfffcff00,0xff30ff00,0xff00ffea,0xfffe7e7e][rank-1] if rank<=5 else 0xffff0000
   color=0xff348bff if 19030<=item<=19050 else default
   ...
   for (hook,end),reg in zip(patches[1:],[UC_X86_REG_EAX,UC_X86_REG_ECX]):
    ...
    u.emu_start(base+hook,base+end,count=100);assert u.reg_read(reg)==(0xff72cfff if 19030<=item<=19050 else color)
    assert u.reg_read(UC_X86_REG_ESP)==stack and u.reg_read(UC_X86_REG_EFLAGS)==flags
   cases+=1
(O/'payload/Engine.dll').write_bytes(b)
(O/'tooltip_validation.json').write_text(json.dumps(dict(passed=True,cases=cases,rank_color='#348BFF',name_color='#72CFFF',item_range=[19030,19050],two_image_bases=True,sha256=hashlib.sha256(b).hexdigest()),indent=2))
