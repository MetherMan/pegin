...
const root=new T.Group();root.name='Bloodfang_Openwork_Spear';
...
const materials={
 ...
 edge:{name:'Honed silver bevel',color:'#aeb4b7',metal:.84,rough:.31},
 ...
 ruby:{name:'Blood-fang ruby',color:'#840b17',metal:.34,rough:.23},
 ...
 fur3:{name:'Blood matted fur',color:'#510d14',metal:0,rough:.57},
 fur4:{name:'Crimson fur tips',color:'#8e1720',metal:0,rough:.49},
};
...
function tube(points,radius,key,name,segments=40,sides=8){return add(new T.TubeGeometry(curve(points),segments,radius,sides,false),key,name);}
...
tube(helix,.00135,'seam','05 | spiral leather binding',1100,5);
...
for(let h=0;h<2300;h++){
 ...
 const width=.0007+rnd()*.0013,baseMat=rnd()<.18?2:rnd()<.60?1:0,bloody=rnd()<.68;
 for(let j=0;j<5;j++){
...
const info={name:'Bloodfang Openwork Spear',triangles:stats.reduce((a,b)=>a+b.triangles,0),mesh_objects:stats.length,chain_links:count,fur_strands:2300,source:'../sketch_concept/blood_beast_spear_openwork_v3.png',construction:'New reference-guided 3D geometry. Pierced extruded blade with thickness and bevels, raised heraldry, toroidal chain links, circular haft and dimensional tapered fur strands.',surface:'Blade uses projected concept texture; some original lighting/detail is baked into this texture. Back uses mirrored front appearance. Other components have PBR materials.',not_in_game:true,parts:stats};
