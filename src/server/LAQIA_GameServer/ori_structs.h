...
#define dMAX_SEND_SOCKBUFF				(dMAX_SOCK_BUFF*50)
...
#define dMAX_MOBINFO					1024	//	최대 몹 정보
...
#define dMOB_DROPITEM_MAX_LINE			1000
#define dMOB_DROMITEM_MAX_MOB			50
...
struct player_quest
{
		int		questNum;
			...
			int		successValue;
				...
				struct player_quest *prev;
					struct player_quest *next;
};

typedef struct player_quest sPLAYER_QUEST, *sPPLAYER_QUEST;
#endif
...
struct quest_data{
	...
	int		questStarItem;					//시작 아이템 번호
	int		questNeedObject;				//퀘스트 반복회수 0 = 반복없음, 1 = 1번, 2 = 2번 .....
	int		questLoop;						//선결퀘스트
	int		questPrequest;					//중복가능 퀘스트
	int		questOverlap;					//수집할 아이템 번호
	int		questItemNum;					//잡을 몬스터번호
	int		questMobNum;					//필요한 아이템 / 몬스터 개수
	int		questEndNpcMap;					//종료NPC맵
	int		questEndNpcNum;					//종료NPC번호
	int		questFinishMode;				//exp보상
	int		questNeedItemMobCnt;			//실드 보상
	int		questCompensation;				//아이템 보상
	int		questExpCompen;	
	int		questMoneyCompen;	
	int		questCar;			
	int		questItem1;		
	int		questItem2;		
	int		questItem3;
...
struct descriptor_data
{
	...
	char sendBuff[dMAX_SEND_SOCKBUFF];				// send 대기중인 데이터 
