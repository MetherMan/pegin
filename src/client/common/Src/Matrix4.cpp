 ...
 *
 * @author	아샬
 * @date	$Date: 03-11-27 6:09p $
 * @version	$Revision: 20 $
 *
 * <HR>
 * $Log: /Common/Src/Matrix4.cpp $
 * 
 * 20    03-11-27 6:09p Kjmgo
 * Z축에 대한 빌보드 수정 ( -atanf를 atanf로 수정 또한 vDir.x가 0일떄의
 * 처리를 삭제 )
 * 
 * 19    03-08-04 11:19a Admin
 * 
 * 18    03-03-17 6:59a Ahastudio
 *
 * 16    03-03-14 5:09p Ahastudio		comment 수정.
 * 15    03-03-09 6:30p Ahastudio		Matrix4x4_Inverse의 Input 인자를 const로 변경.
 * 14    03-03-09 5:25p Ahastudio		가우스 소거법을 이용한 역행렬 구하는 함수 추가.
 * <HR>
 *
 * @file	Matrix4.cpp
 */
 ...
 *
 */
void Matrix4::SetRotationX( const float fAngle )
{
 ...
 *
 */
void Matrix4::SetRotationY( const float fAngle )
{
	float fCos = cosf( fAngle );
	float fSin = sinf( fAngle );
	SetIdentity();
 ...
 *
 */
void Matrix4::SetRotationZ( const float fAngle )
{
	float fCos = cosf( fAngle );
	float fSin = sinf( fAngle );
	SetIdentity();
 ...
 *
 */
void Matrix4::SetRotation( const Vector3& vINAxis, const float fAngle )
{
 ...
 *
 */
void Matrix4::SetView( const Vector3& vPos, const Vector3& vDir_, const Vector3& vUp_ )
{
	Vector3 vDir;
	Vector3 vUp;
	Vector3 vCross;
 ...
 *
 */
void Matrix4::SetProjection(	const float fFOV,			///<
								const float fAspect,		///<
								const float fNearPlane,		///<
								const float fFarPlane		///<
								)
{
	Assert( fabs( fFarPlane - fNearPlane ) > 0.01F );
	Assert( fabs( sin( fFOV / 2 ) ) > 0.01F );
 ...
 *
 */
void Matrix4::SetWorld( const Vector3& vPos )
{
	SetIdentity();
 ...
 *
 */
void Matrix4::SetWorld( const Vector3& vPos, const Quaternion& qRot )
{
	*this = qRot.GetMatrix4();
 ...
 *
 */
void Matrix4::SetWorld( const Vector3& vPos, const Vector3& vINDir, const Vector3& vINUp )
{
 ...
 * 좌표계를 이동한다.
 */
void Matrix4::Translate( const Vector3& vPos )
{
 ...
 * 빌보드 행렬을 얻는다.
 */
Matrix4 Matrix4::GetBillboard() const
{
	Matrix4 matrix;
 ...
 * Y축 중심의 빌보드 행렬을 얻는다.
 */
Matrix4 Matrix4::GetBillboardX() const
{
	...
	return matWorld;
} //Matrix4::GetBillboardY

/**
 * Z축 중심의 빌보드 행렬을 얻는다.
 */
Matrix4 Matrix4::GetBillboardZ() const
{
	static Matrix4	matWorld;
	static Vector3	vDir;
	...
	return matWorld;
} //Matrix4::GetBillboardZ

/**
 * 가우스 소거법을 이용해서 4x4 행렬의 역행렬을 구한다.
 * Intel에서 제공한 스트리밍 SIMD 확장을 이용하지 않은 C 함수.
 *
 * @param	b		[IN]  pointer to array of 16 single floats (source matrix)
 * @param	a		[OUT] pointer to array of 16 single floats (invert matrix)
 */
static void Matrix4x4_Inverse( const float b[][4], float a[][4] )
{
	...
	for (i = 0; i < 4; i++) {
		...
		if (irow != icol) {
			// a[irow][0]과 a[icol][0]을 교환.
			temp = a[irow][0];
			a[irow][0] = a[icol][0];
			a[icol][0] = temp;
			// a[irow][1]과 a[icol][1]을 교환.
			temp = a[irow][1];
			a[irow][1] = a[icol][1];
			a[icol][1] = temp;
			// a[irow][2]와 a[icol][2]를 교환.
			temp = a[irow][2];
			a[irow][2] = a[icol][2];
			a[icol][2] = temp;
			// a[irow][3]과 a[icol][3]을 교환.
			temp = a[irow][3];
			a[irow][3] = a[icol][3];
			a[icol][3] = temp;
		} //if
	...
	if (indxr[3] != indxc[3]) {
		ir = indxr[3];
		ic = indxc[3];
		// a[0][ir]과 a[0][ic]를 교환.
		temp = a[0][ir];
		a[0][ir] = a[0][ic];
		a[0][ic] = temp;
		// a[1][ir]과 a[1][ic]를 교환.
		temp = a[1][ir];
		a[1][ir] = a[1][ic];
		a[1][ic] = temp;
		// a[2][ir]과 a[2][ic]를 교환.
		temp = a[2][ir];
		a[2][ir] = a[2][ic];
		a[2][ic] = temp;
		// a[3][ir]과 a[3][ic]를 교환.
		temp = a[3][ir];
		a[3][ir] = a[3][ic];
		a[3][ic] = temp;
	} //if
