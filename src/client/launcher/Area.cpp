...
BOOL CArea::Init()
{
	...
	m_hFont = CreateFont(	12, 0, 0, 0, 
							FW_NORMAL, 
							FALSE, FALSE, FALSE, 
							IME_CMODE_ALPHANUMERIC,//GB2312_CHARSET, 
							OUT_DEFAULT_PRECIS, 
							CLIP_DEFAULT_PRECIS, 
							ANTIALIASED_QUALITY, 
							VARIABLE_PITCH, 
	...
	SetRect( &AREA_START, 300, 497, 375, 521 );
	SetRect( &AREA_CANCEL, 400, 497, 475, 521 );
	for( i=0 ; i<4 ; i++ )
	{
...
void CArea::SetPage( int nPage )
{
	...
	{
		SetRect( &pAREA_BLOCK[i], 324, 124 + 30 * i, 450, 135 + 30 * i + 23 );
	} // for
...
BOOL CArea::Render()
{
	...
	m_bmAreaBack.Draw( 251, 107, hBackDC );
...
void CArea::DrawBlock( HDC hDC )
{
	...
	for( int i=0 ; i<MAX_AREA ; i++ )
	{
		...
		TextOut( hDC, 368, 129 + 30 * i, sName.GetBuffer( 0 ), sName.GetLength() );
	} // for
