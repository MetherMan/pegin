...
`client-overlay/Tools/BaphometColors/Open.cmd`를 열어 피부 색조·강도·밝기를 조절한 뒤 **피부색 저장**을 누릅니다. 붉은 뿔과 세 눈, 기존 핏자국은 유지됩니다. 이 파일은 색상 편집기이며 게임 실행기는 아닙니다.

저장소에서 저장하면 배포 텍스처·설정·manifest 해시가 함께 갱신됩니다. 커밋·푸시 후 아버지는 기존 `자동업데이트.cmd`로 받고, 평소 outputs의 **혼자 게임 시작**을 사용합니다. 아버지 설치본에서 직접 편집하려면 `work/laqia-runtime/client/GameClient/Tools/BaphometColors/Open.cmd`를 사용합니다. 이후 자동업데이트는 저장소에서 배포한 색으로 맞춥니다.
